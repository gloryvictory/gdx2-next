...
import { getFields } from "../actions/getAll";
...
export default  function GetField() {
    // const items : IResult = await getFields();
    
  const title: string  = "Отчеты  | Месторождения"
...
useEffect(() => {
  ...
  return (
    ...
    { isLoading && <Spin tip="Получаем все месторождения..." size="large" fullscreen/>  } 

    <div className="mt-20">
      <h1>Месторождений: <strong>{data?.count}</strong> </h1>
